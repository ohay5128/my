...
function convertToQYWXAM(configs) {
    const configStrings = configs.map(config => {
        // 检查所有字段是否都为空
        const allEmpty = !config.corpId && !config.agentId && !config.secret && !config.type;
        
        // 如果都为空则返回空字符串，否则返回拼接的字符串
        return allEmpty ? "" : `${config.corpId},${config.agentId},${config.secret},${config.type}`;
    });
    ...
    return new Promise((resolve) => {
        ...
        if (FSKEY) {
            const options = {
                ...
                json: { msg_type: 'text', content: { text: desp ? `${text}\n\n${desp}` : text } },
                headers: {
                    'Content-Type': 'application/json',
                },
                timeout,
